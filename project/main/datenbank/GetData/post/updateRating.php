...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($userId !== null && $postId !== null) {
        ...
        if ($userPost && $userPost['post_id'] == $postId) {
            if (updatePostRating($postId, $ratingValue)) {
                header("Location: ../../../pages/post/post.php?manga_id=" . intval($mangaId) . "&chapter_id=" . intval($chapterId));
                exit();
            } else {
                echo "Fehler beim Aktualisieren des Ratings: " . mysqli_error($GLOBALS['conn']);
            }
        } else {
            echo "Sie können nur Ihr eigenes Post-Rating aktualisieren.";
        }
    } else {
